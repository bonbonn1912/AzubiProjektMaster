...
public class Window_Graph1 : MonoBehaviour
{
   ...
   // public static List<int> valueList6 = new List<int>() { 10, 100 };
    public int daybefore = GlobalVariables.day;
     int switcherWIndow = 2;
    private List<int> Locallist = Window_Graph.valueList2;
    private void Awake()
    {
        Debug.Log("klassen geladen");
    ...
    private void Update()
    {
        ...
        {
            Debug.Log("day in windowgraph 2 changed");
            ...
            {
                Locallist.Reverse();
                Debug.Log("window 2 wird gezeichnet");
                ShowGraph(Locallist, -1, (int _i) => "" + (_i + 1), (float _f) => "$" + Mathf.RoundToInt(_f));
